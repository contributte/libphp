...
test('count files', async () => {
  expect(Object.keys(await index.getFiles()).length).toEqual(115);
  expect(Object.keys(await index.getPhpFiles()).length).toEqual(6);
  expect(Object.keys(await index.getPhpModulesFiles()).length).toEqual(51);
  expect(Object.keys(await index.getSharedLibsFiles()).length).toEqual(58);
});
