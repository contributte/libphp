...
test('count files', async () => {
  ...
  const phpFiles = await index.getPhpFiles();
  const modulesFiles = await index.getPhpModulesFiles();
  const libFiles = await index.getSharedLibsFiles();

  expect(Object.keys(files).length).toEqual(Object.keys(phpFiles).length + Object.keys(modulesFiles).length + Object.keys(libFiles).length);
  expect(Object.keys(phpFiles).length).toEqual(6);
});
